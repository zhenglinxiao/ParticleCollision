...
public class QuantumFormulas {
	...
	public static ArrayList<Particle> getJetParticles(Particle p) {
		...
		int strangeNum = 0;
		int charmedNum = 0;
		Particle pion = new Particle(zero, zero,zero,5,"pion");
		Particle kaon = new Particle(zero, zero,zero,5,"kaon");
		Particle dMeson = new Particle(zero, zero,zero,5,"dMeson");
		Particle strangeDMeson = new Particle(zero, zero,zero,5,"");
		Particle charmedBMeson = new Particle(zero, zero,zero,5,"pion");

		if (energy <= pion.getMinimalEnergy()) {
			pionNum = jetNum;
		} else if (energy <= kaon.getMinimalEnergy()) {
			pionNum = (int) (0.75 * jetNum);
			kaonNum = jetNum - pionNum;
		} else if (energy <= dMeson.getMinimalEnergy()) {
			pionNum = (int) (0.6 * jetNum);
			kaonNum = (int) (0.25 * jetNum);
			dMesonNum = jetNum - pionNum - kaonNum;
		} else if (energy <= strangeDMeson.getMinimalEnergy()) {
			pionNum = (int) (0.50 * jetNum);
			kaonNum = (int) (0.20 * jetNum);
			dMesonNum = (int) (0.18 * jetNum);
			strangeNum = jetNum - dMesonNum - pionNum - kaonNum;
		} else if (energy <= charmedBMeson.getMinimalEnergy()) {
